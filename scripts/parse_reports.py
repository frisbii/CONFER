...
def process_utilization(util_dict):
    ...
    for primitive_type in util_dict:
        if primitive_type["Functional Category"] in capture:
            categories[capture[primitive_type["Functional Category"]]] += int(
                primitive_type["Used"]
            )
        else:
...
def wrangle_dataframe(df):
    ...
    df = (
        df.set_index(["operation", "datatype", "part", "period", "width"])
        .sort_index()
        .loc[
            :,
            ["delay_route", "delay_logic", "power_static", "power_dynamic"]
            + list(existing_primitives),
        ]
    )
    return df
...
def main():
    ...
    for rpt in rpt_dir.iterdir():
        ...
        with rpt.open() as f:
            ...
            match rpt_type:
                case "utilization":
                    row["primitives"] = vrp.parse_vivado_report(raw_contents)[
                        "Primitives"
                    ]
                case "power":
                    summary_dict: dict = vrp.parse_vivado_report(raw_contents)[
                        "Summary"
                    ]
                    try:
                        row["power_total"] = first_float(
                            summary_dict["Total On-Chip Power (W)"]
                        )
                        row["power_static"] = first_float(
                            summary_dict["Device Static (W)"]
                        )
                ...
                case "timing":
                    match = re.search(
                        r"^\s*Data Path Delay:\s*([0-9\.]*)ns\s*\(logic ([0-9\.]*)ns [0-9\.%\(\)]*  route ([0-9\.]*)ns",
                        raw_contents,
                        re.MULTILINE,
                    )
                    if match is None:
    ...
    df = (
        df.groupby(["datatype", "operation", "width", "part", "period"])
        .aggregate("first")
        .reset_index()
    )
